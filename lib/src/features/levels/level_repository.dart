...
class LevelRepository {
  ...
  void _validateLevels() {
    for (var level in _levels) {
      if (level.gridSize != 36)
        debugPrint('Warning: Level ${level.id} grid size mismatch');
      ...
      for (var q in level.questions) {
        ...
        if (q.answer.length != q.answerPlacement.path.length) {
          debugPrint(
            'Error: Level ${level.id} Q${q.qId} answer length mismatch',
          );
        }
        ...
        if (gridWord.toLowerCase() != q.answer.toLowerCase()) {
          debugPrint(
            'Error: Level ${level.id} Q${q.qId} grid mismatch. Found "$gridWord", expected "${q.answer}"',
          );
        }
      }
    }
