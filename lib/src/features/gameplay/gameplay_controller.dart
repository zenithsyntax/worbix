...
class GameplayController extends StateNotifier<GameplayState> {
  ...
  Future<void> loadLevel(int levelId) async {
    ...
    if (userProgress.lastPlayedLevelId == levelId) {
      ...
      while (startIndex < level.questions.length &&
          completed.contains(level.questions[startIndex].qId)) {
        startIndex++;
      }
    ...
    state = GameplayState(
      ...
      currentCoins:
          0, // This is session coins? Or total? "Total coins earned (from all its questions)".
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      state = state.copyWith(
        currentQuestionDuration: state.currentQuestionDuration + 1,
      );
  ...
  void clearSelection() {
    state = state.copyWith(selectedIndices: [], hintPositions: {});
  }
  ...
  void selectHintLetterAtPosition(
    List<Map<String, int>> pathUpToHint,
    int hintPositionIndex,
  ) {
    if (state.status != GameStatus.playing) return;
  ...
  void onTileTap(int index) {
    ...
    if (state.selectedIndices.isEmpty) {
      ...
      state = state.copyWith(selectedIndices: [index], hintPositions: {});
      return;
    }
    ...
    if (state.selectedIndices.contains(index)) {
      ...
      if (state.selectedIndices.length > 1 &&
          index == state.selectedIndices[state.selectedIndices.length - 2]) {
        final newSelection = List<int>.from(state.selectedIndices)
          ..removeLast();
        state = state.copyWith(selectedIndices: newSelection);
        return;
  ...
  Future<void> _checkAnswer() async {
    ...
    if (word.toLowerCase() == q.answer.toLowerCase()) {
      ...
      if (state.isTimeExpired) {
        ...
        coinsEarned = (maxCoins * (timeLeft / timeLimit))
            .clamp(1.0, maxCoins.toDouble())
            .toInt();
      }
      ...
      if (state.currentQuestionIndex + 1 < state.level!.questions.length) {
        await ref
            .read(userProgressProvider.notifier)
            .savePosition(state.level!.id, state.currentQuestionIndex + 1);
      }
  ...
  Future<void> nextQuestion() async {
    ...
    if (state.currentQuestionIndex + 1 >= state.level!.questions.length) {
      ...
      await ref
          .read(userProgressProvider.notifier)
          .unlockNextLevelOnCompletion(state.level!.id, allQuestionIds);
      ...
      ref
          .read(userProgressProvider.notifier)
          .savePosition(state.level!.id, nextIndex);
  ...
  void jumpToQuestion(int index) {
    if (state.level == null ||
        index < 0 ||
        index >= state.level!.questions.length)
      return;
...
final gameplayControllerProvider =
    StateNotifierProvider.autoDispose<GameplayController, GameplayState>((ref) {
